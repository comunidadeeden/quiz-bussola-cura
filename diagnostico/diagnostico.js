...
const ENV_CONFIG = {
  ...
  liveLinkUrl: window.NEXT_PUBLIC_LIVE_LINK_URL || OFFER_CONFIG.liveLinkUrl,
...
function renderResult() {
  ...
  document.querySelectorAll("[data-live-link]").forEach((item) => item.addEventListener("click", handleLiveLinkClick));
}
...
function ctaWorkshop(result) {
  return html`
    <section class="screen cta-card">
      <span class="eyebrow">Live ao vivo</span>
      <h2>Agora leve seu diagnóstico para o Workshop Bússola da Cura.</h2>
      <p class="pitch-lead">Agora você sabe qual função emocional apareceu com mais força no seu rastreio. <mark>Na live, Bruno vai mostrar como começar a investigar a raiz desse padrão.</mark></p>
      <div class="pitch-box">
        <strong>${OFFER_CONFIG.dateText}, às ${OFFER_CONFIG.timeText}</strong>
        <p>Uma aula prática, ${OFFER_CONFIG.formatText}, para entender a relação entre sintomas, fase de vida, padrão emocional e próximos passos de autoconsciência.</p>
      </div>
      <div class="pitch-grid">
        <div>
          <strong>Na live você vai entender</strong>
          <ul class="feature-list">
            <li>Como sair do sintoma isolado e enxergar o padrão.</li>
            <li>O que observar na fase em que o sintoma começou ou voltou.</li>
            <li>Como usar seu resultado como ponto de partida.</li>
          </ul>
        </div>
        <div>
          <strong>Seu próximo passo</strong>
          <ul class="feature-list">
            <li>Receber o link de acesso à live.</li>
            <li>Chegar com seu diagnóstico em mãos.</li>
            <li>Acompanhar a explicação do Bruno ao vivo.</li>
          </ul>
        </div>
      </div>
      <p class="pitch-lead">Clique abaixo para registrar seu interesse e receber o link do workshop ao vivo.</p>
      <a class="button gold full" href="${buildLiveLinkUrl()}" data-live-link>Quero receber o link da live</a>
      <div class="checkout-meta">
        <span id="live-link-message">Live online · ${OFFER_CONFIG.dateText} · ${OFFER_CONFIG.timeText}</span>
      </div>
    </section>
  `;
}
...
function buildLiveLinkUrl() {
  if (!ENV_CONFIG.liveLinkUrl) return "#";
  const url = new URL(ENV_CONFIG.liveLinkUrl);
  const currentParams = new URLSearchParams(window.location.search);
...
function handleLiveLinkClick(event) {
  event.preventDefault();
  const url = buildLiveLinkUrl();
  trackEvent("quiz_live_link_click", { result: state.result, liveLinkUrl: url });
  trackEvent("quiz_cta_click", { result: state.result, cta: "live_link" });
  postLead({ event: "live_link_requested", lead: state.lead, answers: state.answers, result: state.result });

  if (ENV_CONFIG.liveLinkUrl) {
    window.location.href = url;
    return;
  }

  const message = document.querySelector("#live-link-message");
  if (message) message.textContent = "Pedido registrado. Você receberá o link nos seus contatos.";
}
