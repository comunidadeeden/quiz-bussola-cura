const RAIOX_CONFIG = {
  ...
  leadWebhookUrl: "https://script.google.com/macros/s/AKfycbzLDny-WNXvTEQ0G5mpjl-cdnAzSauQf8-mPWFxeL3OYtuQrNi2gcDtbg3zQgho4yFi/exec",
  source: "quiz_raiox01v3",
...
function handleLeadSubmit(event) {
  ...
  state.lead = lead;
  localStorage.setItem("raiox01v3_lead", JSON.stringify(lead));
  sendLeadEvent("lead_submitted");
  trackEvent("raiox01v3_lead_submit");
  state.screen = "step";
  render();
}
...
function renderInsight(step, progress) {
  ...
  `);
  document.querySelector("#continue-button").addEventListener("click", () => { state.stepIndex += 1; trackEvent("raiox01v3_insight_continue", { insight: step.id }); render(); });
}
...
function answerStep(step, optionIndex, button) {
  ...
  trackEvent("raiox01v3_answer", { question: step.id, profile: state.profile });
  window.setTimeout(() => {
    state.stepIndex += 1;
...
function renderLoading() {
  ...
  window.setTimeout(() => {
    ...
    trackEvent("raiox01v3_result_view", { profile: state.profile });
    render();
  }, 750);
}
...
function renderResult() {
  ...
  document.querySelector("#checkout-button").addEventListener("click", () => {
    ...
    trackEvent("raiox01v3_checkout_click", { profile: state.profile, vsl_profile: vslProfile });
  });
}
...
function createSubmissionId() {
  ...
  return `raiox01v3-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
}
