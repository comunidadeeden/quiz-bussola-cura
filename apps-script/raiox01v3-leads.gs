const RAIOX01V3_LEADS_CONFIG = {
  spreadsheetId: "1OBr2lZO_AyVS30f2_KD0qwLtrBxyY-8owmbVCq50KK4",
  sheetName: "Leads Raio X 01 V3",
  submissionIdColumn: "submission_id"
};

function doGet() {
  return response_({ ok: true, message: "Webhook exclusivo do Quiz Raio X 01 V3 ativo." });
}

function doPost(event) {
  try {
    const payload = event && event.postData && event.postData.contents ? JSON.parse(event.postData.contents) : {};
    const spreadsheet = SpreadsheetApp.openById(RAIOX01V3_LEADS_CONFIG.spreadsheetId);
    const sheet = spreadsheet.getSheetByName(RAIOX01V3_LEADS_CONFIG.sheetName);
    if (!sheet) throw new Error("Aba Leads Raio X 01 V3 não encontrada.");
...
function findRow_(sheet, headers, submissionId) {
  const idColumn = headers.indexOf(RAIOX01V3_LEADS_CONFIG.submissionIdColumn) + 1;
